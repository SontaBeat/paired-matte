...
**使用前须准备好一组对齐的单色背景彩色图与灰度遮罩。网页工具不直接从复杂背景原图中识别、选择或分离某个物体，也不会自行生成遮罩。** 本项目提供的是配对素材准备流程与后续透明合成工具，不是“上传任意原图即可一键去背景”的自动分割工具。
...
## 为什么做这个项目
在一些需要高精度的抠图场景下（如复杂背景下的头发丝、动物毛发等，或者是半透明材质如光效、投影、薄纱等），用传统的图像处理工具（如Photoshop）或专门的抠图工具等，往往并不能取得很好的效果并且需要花费大量时间，无法批量处理，每一张图片都要精修。即使在gpt image2.5如今能够准确生成透明通道图的情况下，分离出来的图片在边缘或者原图的一致性方面也无法做到精准的效果。
我们的实践是先生成一张已知纯色底的彩色图，再以这张图为唯一几何参考生成灰度 Alpha，最后在浏览器中去除背景混色，并检查不同底色上的结果。
...
- **两张输入需要先行准备**：使用者需要自行或借助支持本流程的代理，准备单色背景彩色图及与其位置、尺寸、裁切一致的灰度遮罩。仓库中的示例可直接使用，但网页不会替其他原图自动完成这一步。
- **不负责复杂场景中的物体分割**：网页没有目标识别、点选物体或语义分割功能。复杂背景的去除、主体选择和遮挡区域重建发生在前面的生图或素材制作阶段，而非网页的 Alpha 合成阶段。
- **不能凭空补回被遮挡的原始信息**：示例中去除木台、保留纱物的过程包含模型估计和重建，不代表从原图中无损提取出了不可见的纱物颜色或结构。
- **结果受配对素材质量约束**：错误遮罩、轮廓错位、非均匀底色以及半透明区域中残留的旧背景，会传递到最终 PNG。网页调节不能替代合格输入，也不能自动修复几何错误。
- **准备阶段存在时间与服务成本**：使用 ChatGPT 网页版需账号、网络和可用额度，生成可能需重试；本地网页本身离线运行。无需专业软件不等于无需准备、无需验收或全程零成本。
- **批量是流程组织，不是一键全自动**：每张图仍需配对生成与质量检查。当前网页一次处理一组图片，不提供文件夹批处理、自动对齐或自动质量判定。

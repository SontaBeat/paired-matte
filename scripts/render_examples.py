...
for key, title in titles.items():
    ...
    size = manifest['size']
    route = '2026-09-17 网页版生成配对图，Safari 本地工具导出' if manifest.get('web_end_to_end_rerun') else '历史配对图，本次网页重新导出'
...
html = '''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Paired Matte · 中文草稿审阅</title><style>body{margin:0;background:#f3f5f2;color:#20322f;font:16px/1.8 system-ui,-apple-system,sans-serif}main{max-width:1300px;margin:0 auto;padding:44px 24px}header,section{background:white;border:1px solid #d9e2db;border-radius:14px;padding:28px;margin-bottom:22px}h1{font-size:36px;line-height:1.2}h2{font-size:23px}a{color:#006c72}img{width:100%;height:auto;border:1px solid #ddd}small{color:#56665f}.tag{display:inline-block;padding:3px 12px;background:#fff2d2;border-radius:12px}.flow{padding:18px;background:#edf5f1;border-radius:8px}nav{display:flex;gap:20px;flex-wrap:wrap}</style><main><header><span class="tag">私有审阅稿 · 未公开发布</span><h1>Paired Matte<br>配对遮罩半透明抠图</h1><p>ChatGPT 网页生图流程、可复制 Skill、本地网页与历史案例。</p><div class="flow">原图 → 纯色底图 → 以纯色图生成 Alpha → 对齐检查 → 去底色与微调 → 透明 PNG</div><p>三组演示可查看完整文件。模型 Alpha 没有真值；半透明与光效的质量需在目标背景下判断。</p><nav><a href="cutout-tool.html">打开网页工具</a><a href="README.md">中文 README</a><a href="REVIEW.md">审阅清单</a><a href="skills/gpt-image-2-subject-assets/SKILL.md">查看 Skill</a></nav></header>''' + ''.join(cards) + '''<footer><small>代码、Skill 与文档采用 GPL-3.0-or-later；演示素材由作者制作并授权用于项目演示。私有审阅版本，未部署网页或发布 Release。</small></footer></main></html>'''
